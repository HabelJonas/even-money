...
fun ExpenseDetailsScreen(
    ...
    LaunchedEffect(Unit) {
        ...
        expenseRepository.readExpense(groupId, title) { success, expense ->
            if (success) {
                expenseTitle = expense.title
                euroAmount = expense.amount.toString()
                selectedPayer = expense.paidBy
                notes = expense.notes
                memberDistributions = expense.distributions
                for (entry in expense.distributions) {
                    memberSelections =
                        memberSelections.toMutableMap().apply { this[entry.key] = true }
                }
            }
        }
    }
                ...
                Row(
                    ...
                    Checkbox(
                        ...
                        onCheckedChange = { isChecked ->
                            memberSelections =
                                memberSelections.toMutableMap().apply { this[member] = isChecked }
                        }
                    )
                    Text(text = member)
...
fun ExpenseDetailsPreview() {
    ExpenseDetailsScreen(
        navController = rememberNavController(), groupId = "", title = "",
        expenseRepository = FakeExpenseRepository(), groupRepository = FakeGroupRepository()
    )
}
